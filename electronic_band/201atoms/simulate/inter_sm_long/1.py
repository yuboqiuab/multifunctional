...
class NNDataset(Dataset):
    def __init__(self):
        ...
        with open('all_vector', 'r') as f:
            self.xyz = np.fromstring(f.read(), sep='\n').reshape(-1, 4)
...
class NNModel(nn.Module):
    ...
    #    self.fc3 = nn.Linear(16, 6)
        self.mc1 = nn.Linear(4,32)
        self.mc2 = nn.Linear(32,16)
        self.mc3 = nn.Linear(16,15)
...
def train(model, device, train_loader, loss_criterion, accu_criterion, optimizer, epoch, logwandb):
    ...
    for i, (data, xyz, target) in enumerate(train_loader):
...
def validate(model, device, test_loader, loss_criterion, accu_criterion, logwandb, test=False, normalizer=None):
    ...
    with torch.no_grad():
        ...
        if test:
            y_pred, y_true = [], []
        for i, (data, xyz, target) in enumerate(test_loader):
...
def main():
    ...
    parser.add_argument('--momentum', default=0.9, type=float, metavar='M',
                        help='momentum')
    parser.add_argument('--weight-decay', default=1e-7, type=float,
