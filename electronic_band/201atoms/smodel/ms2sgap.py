...
class NNDataset(Dataset):
    def __init__(self):
        ...
        with open('../inputms2sgap_1', 'r') as f:
...
def main():
    ...
    def hook_fn(module, input, output):
        ato_env_rep.append(output.detach().cpu().numpy())

    model.mc2.register_forward_hook(hook_fn)

    pred, target = validate(model, device, val_loader, loss_criterion, accu_criterion, logwandb=False, test=True)
    ato_env_rep = np.vstack(ato_env_rep)


    np.savetxt('reduce_inputms2s', np.hstack([ato_env_rep]), fmt=  ' %9.6f' * 32, delimiter='  ')
